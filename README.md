...
SnapClass runs on Supabase (PostgreSQL). The relationships are structured as follows:
...
erDiagram
    ...
    subjects ||--o{ attendance_logs : "records in"
    students ||--o{ attendance_logs : "registers"
```

### Applying Schema Constraints
The join table `subject_students` uses a **composite primary key** `(student_id, subject_id)` to allow many students to enroll in the same subject, while enforcing that a student can only enroll in a subject once.

Run the script in [20260705000000_fix_enrollment_constraints.sql](file:///Users/shravanmole/Documents/snapclass/supabase/migrations/20260705000000_fix_enrollment_constraints.sql) inside the **Supabase SQL Editor** to initialize constraints and configure Row Level Security (RLS) policies.
...
## 🛠️ API & Database Functions Reference

### `create_teacher(username, password, name)`
Inserts a new teacher record with hashed password into the database.
*   **Parameters**: `username` (str), `password` (str), `name` (str)
*   **Returns**: list (created row metadata)

### `teacher_login(username, password)`
Queries teacher profiles and validates credentials.
*   **Parameters**: `username` (str), `password` (str)
*   **Returns**: dict (teacher data) or `None`

### `create_student(name, face_embedding, voice_embedding)`
Registers a student with high-dimensional facial and vocal embedding arrays.
*   **Parameters**: `name` (str), `face_embedding` (list of float), `voice_embedding` (list of float)
*   **Returns**: list (student row metadata)

### `get_teacher_subjects(teacher_id)`
Fetches subjects taught by a teacher, dynamically aggregating student counts and total class count metrics.
*   **Parameters**: `teacher_id` (int)
*   **Returns**: list of dict

### `enroll_student_to_subject(student_id, subject_id)`
Creates an enrollment relation in the join table.
*   **Parameters**: `student_id` (int), `subject_id` (int)
*   **Returns**: list

---

## ⚠️ Known Limitations
*   **Low Light Biometrics**: FaceID scans may perform poorly in low light conditions where camera contrast is low.
*   **Installation Overhead**: Compiling `dlib` binaries from source requires system compiler tools, which may make the initial development setup complex on some systems.
*   **Database URL Prefix Suffix**: PostgREST requires clean endpoint URLs (without `/rest/v1` suffix) to prevent PGRST125 path issues.
...
3.  Click **New app**, select your repository (`snapclass`), set branch to `master`, and main file path to `app.py`.
4.  Expand **Advanced settings...** and paste your `.streamlit/secrets.toml` variables.
5.  Click **Deploy**.
